...
/*	jsonから取得に変更
	...
	*/
	...
	const $kagoDoc = $kago.contentDocument;

	const $kagoTable = $kagoDoc.getElementsByTagName('table');
	const $kagoTT = $kagoTable[0].getElementsByClassName('ttprice');
	let   $kagoDell = $kagoDoc.getElementsByTagName('a');
	const $kagobill = $kagoDoc.getElementsByClassName('bill');

	const serverAdd = "http://192.168.1.103:3000/";
...
//------------------------------------------------------------------------------
	...
	const getTrLen = () =>{
		return $kagoTable[0].getElementsByTagName('tr').length;
	};
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------

	const lastTrpricesubs = () =>{
			//price
			let _last = getTrLen()-1;
			let _price = $kagoTable[0].getElementsByTagName('tr')[_last].childNodes[1].textContent;

				//合計金額減算
			ttprice -= parseInt(_price.slice(1));
			$kagoTT[0].textContent = ttprice;
	}
...
//------------------------------------------------------------------------------
	...
	const remRow =()=>{
		...
		if(getTrLen() > 1){
			...
			lastTrpricesubs();
			//最終行削除
			$kagoTable[0].tBodies[0].deleteRow(-1);
		}
	}
...
let Js;
	...
	if (1 === 2){
		Js = new JsonC("./json/susiB.json");
	}
	else
	{
		Js = new JsonC("http://192.168.1.103:3000/");
...
//	cataloges3 =  1 === 1 ? getJSON():Js.getJSON();
	...
	cataloges3 = cataloges3;

	// cataloges3 = Js.getJSON();
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
/*	const sendMail =
	Email.send({
...
//  画像配置処理
	const dispMatrix =(_point)=>{
		...
		//かご イベント
			...
			//かご イベント row
			if(Isfirst){
				$kagoDell[0].addEventListener("click",(e)=>{
					remRow();
				});
				$kagobill[0].addEventListener("click",(e)=>{
					console.log("bill",$kagobill[0])
//					bill(serverAdd + "bill",makeBill());
					bill(serverAdd + "",makeBill());
				});
			}
			Isfirst = false;
		};
